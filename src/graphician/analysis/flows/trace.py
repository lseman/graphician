...
def _trace_flow(
    ...
    """Forward BFS from entry through Calls edges, bounded by depth and size."""
    safety_ceiling = max(options.max_nodes_per_flow * 10, 500)
    visited: set[NodeId] = {entry}
    members: list[tuple[NodeId, int]] = []
    queue: deque[tuple[NodeId, int]] = deque([(entry, 0)])

    # Phase 1: uncapped BFS up to safety ceiling.
    while queue:
