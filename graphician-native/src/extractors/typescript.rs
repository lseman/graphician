...
//! Emits: File, Class, Interface, TypeAlias, Enum, Function, Method, Module
...
pub fn extract_typescript_file(
    py: Python,
    source: &[u8],
    file_path: &str,
    file_qn: &str,
    ...
    // Walk root and emit declarations
    ...
    for child in root.children(&mut cursor) {
        match child.kind() {
            ...
            "interface_declaration" => {
                emit_interface(&child, &file_qn_full, file_path, source, &mut result)
            }
            "type_alias_declaration" => {
                emit_type_alias(&child, &file_qn_full, file_path, source, &mut result)
            }
            "enum_declaration" => emit_enum(&child, &file_qn_full, file_path, source, &mut result),
            ...
            "export_statement" | "export_declaration" => {
                if let Some(decl) = child.child_by_field_name("declaration") {
                    match decl.kind() {
                        ...
                        "interface_declaration" => {
                            emit_interface(&decl, &file_qn_full, file_path, source, &mut result)
                        }
                        "type_alias_declaration" => {
                            emit_type_alias(&decl, &file_qn_full, file_path, source, &mut result)
                        }
                        "enum_declaration" => {
                            emit_enum(&decl, &file_qn_full, file_path, source, &mut result)
                        }
                        _ => {}
                    }
                }
            ...
            "method_definition" => {
                // Top-level method-like (in class body handled by emit_class)
                emit_method(
                    &child,
                    &file_qn_full,
                    file_path,
...
fn emit_class(
    ...
    let mut props = vec![("dialect".to_string(), "typescript".to_string())];
