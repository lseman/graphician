"""Daemon and git hooks installation for Ariadne.
...
def cmd_install(db_path: str, repo: str, force: bool = False,
    ...
    Args:
        ...
        agents: Install AGENTS.md with Ariadne instructions.
    ...
    for hook_name in ["post-commit", "post-merge", "post-checkout"]:
        ...
        script = (
            f'#!/bin/sh\n'
            f'"{sys.executable}" -m ariadne_py --db "{db_path}" update '
    ...
    if agents:
        ...
        block = (
            f"# Ariadne Agent Instructions\n\n"
            f"- Start exploration with `ariadne --db {db_path} tool "
            f'minimal_context --params \'{{"target":"...","mode":"review"}}\'`.\n'
            f"- For code review, run `ariadne --db {db_path} tool "
            ...
            f"- Fall back to direct file reads only after Ariadne identifies the "
...
def _install_mcp_configs(
    ...
    if not claude_path.exists():
        claude_path.write_text(
            f"# Ariadne MCP\n\n"
            f"Use the ariadne MCP tool for codebase exploration.\n"
            f"Database: {db_path}\n",
            encoding="utf-8",
        )
    ...
    codex_path = codex_dir / "ariadne-mcp.toml"
    toml_content = (
        f'[command.ariadne-mcp]\n'
        f'command = "{exe}"\n'
        f'args = ["-m", "ariadne_py", "--db", "{db_path}", "mcp-server"]\n'
        f'description = "Ariadne code graph MCP server"\n'
    )
