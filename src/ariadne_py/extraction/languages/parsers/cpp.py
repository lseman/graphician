...
def _walk_body(
    ...
    for child in _children(node):
        if child.type == "include":
            ...
            _handle_struct(child, source, graph, file_qn, parent_id, scope)
        elif child.type == "function_definition":
            _handle_function(child, source, graph, file_qn, parent_id, scope, False)
        elif child.type == "declaration":
            _handle_declaration(child, source, graph, file_qn, parent_id, scope)
...
def _handle_namespace(
    ...
    if body:
        _walk_body(body, source, graph, file_qn, mod_id, child_scope)
...
def _handle_struct(
    ...
    body = node.child_by_field_name("body")
    if body:
        _walk_class_body(body, source, graph, file_qn, struct_id, child_scope)
...
def _walk_class_body(
    ...
    for child in _children(node):
        if child.type == "function_definition":
            _handle_function(child, source, graph, file_qn, parent_id, scope, True)
        elif child.type == "declaration":
            _handle_declaration(child, source, graph, file_qn, parent_id, scope)
