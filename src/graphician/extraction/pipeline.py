...
class ExtractionPipeline:
    ...
    def _walk_inheritance(
        self, node: Any, source: str, file_key: str, class_qname: str | None = None,
    ) -> None:
        """Find inheritance declarations.

        Tracks the current class node so that INHERITS edges point from
        the class to its bases (matching ariadne-rust's emit_python_bases
        behaviour), instead of from the file node.
        """
        node_type = node.type

        # Python: class Foo(Bar, Baz)
        if node_type == "class_definition":
            # First, find the class name node
            class_name = None
            for child in node.children:
                if child.type in ("identifier", "name"):
            ...
            if class_name is not None:
                class_qname = f"{file_key}::{class_name}"
                ...
                if child.type == "parameters":
                    for base in child.children:
                        if base.type == "identifier" or base.type == "dotted_name":
                            ...
                            # Use type:: prefix for base classes (matches ariadne-rust)
        ...
        for child in node.children:
            self._walk_inheritance(child, source, file_key, class_qname)
